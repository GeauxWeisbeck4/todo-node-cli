...
export default async function updateTask() {
    try {
        // Obtaining the task code entered by user by calling
        // getTaskCode() method
        ...
        // Checking if the todos exist or not
        if(!todo) {
            console.log(chalk.redBright('Could not find a todo with the code you provided'));
        } else {
            console.log(chalk.blueBright('Type the updated properties. Press Enter.'));

            // Get the user's response of the updated data
            const update = await askUpdateQ(todo);

            // If user maked status as completed, we delete the
            // todo, else we update the status
            if(update.status === 'completed') {
                ...
                // Stopping the spinner and display the success
                spinner.stop();
